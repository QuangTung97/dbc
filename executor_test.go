...
func (e *executorTest) newExecMySQL5x() *Executor[tableTest03] {
	exec, err := NewExecutor(DialectMySQL5x, e.schema)
	if err != nil {
		panic(err)
	}
	return exec
...
func TestExecutor_MySQL__SelectCond__Order_By_Only(t *testing.T) {
	e := newExecTest(t)
	exec := e.newExecMySQL()
	...
	// check args
	assert.Equal(t, 1, len(e.selectArgs))
	assert.Equal(t, []any(nil), e.selectArgs[0])
}
